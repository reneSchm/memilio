                             ...
                             const std::vector<std::string>& state_names)
{
    ...
    //     fprintf(outfile, " %s", state_names[k].c_str());
    // }
    // auto num_points = static_cast<size_t>(results.get_num_time_points());
    // for (size_t i = 0; i < num_points; i++) {
    //     fprintf(outfile, "\n%.14f", results.get_time(i));
    //     Eigen::VectorXd res_j = results.get_value(i);
    //     for (size_t j = 0; j < state_names.size(); j++) {
    //         fprintf(outfile, " %.14f", res_j[j]);
    //     }
    // }
    //print column labels
    ...
    for (size_t k = 0; k < static_cast<size_t>(results.get_num_elements()); k++) {
        if (k < state_names.size()) {
            ...
            fprintf(outfile, " %s", ("#" + std::to_string(k + 1)).data());
        }
    }
