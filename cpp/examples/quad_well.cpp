...
int main(int argc, char** argv)
{
    ...
    for (auto& a : agents) {
        switch (agent_itr % 4) {
        case 0:
            // a.position          = {pos_rng(-1.8, 0.2), pos_rng(0.2, 1.8)};
            a.position          = {-1, 1};
            ...
            break;
        case 1:
            // a.position = {pos_rng(0.2, 1.8), pos_rng(0.2, 1.8)};
            a.position = {1, 1};
            a.status   = Status::S;
            break;
        case 2:
            // a.position = {pos_rng(-1.8, 0.2), pos_rng(-1.8, 0.2)};
            a.position = {-1, -1};
            a.status   = Status::S;
            break;
        case 3:
            // a.position = {pos_rng(0.2, 1.8), pos_rng(-1.8, 0.2)};
            a.position = {1, -1};
            a.status   = Status::S;
            break;
        }
    ...
    std::vector<std::vector<ScalarType>> populations = {{n_agents / 4.0 * pop_dist[0], n_agents / 4.0 * pop_dist[1], 0},
                                                        {n_agents / 4.0, 0, 0},
                                                        {n_agents / 4.0, 0, 0},
                                                        {n_agents / 4.0, 0, 0}};
