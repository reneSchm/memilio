...
void run_hybridization(size_t num_runs, bool save_percentiles, std::string result_path)
{
    ...
    size_t num_agents = 4000;

    QuadWellSetup<ABM::Agent> setup(num_agents);
...
#pragma omp single
    //post processing
    {
        ...
        if (save_percentiles) {
            auto ensemble_percentile = mio::mpm::paper::get_format_for_percentile_output(ensemble_results, num_regions);
...
int main()
{
    ...
    run_hybridization(100, true, mio::base_dir() + "cpp/outputs/QuadWell/test/Test7_Hybrid_");
    return 0;
}